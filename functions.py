...
def get_random_window_range(size_, range_of_length_, fitting_length_, n_range_):
    ...
    count2 = 0
    while True:
        pos_ = int(np.random.random() * size_)
        length_ = int((np.random.random()*2. - 1.) * range_of_length_) + fitting_length_
        if [pos_, pos_ + length_] not in bounds_:
            if pos_ + length_ < size_:
                bounds_.append([pos_, pos_ + length_])
                window_.append(np.arange(pos_, pos_ + length_))
                count += 1
        if count == n_range_:
            break
        count2 += 1
...
def aic_fit_bounds(x_, y_, fcns_, p0_, window_, key_):
    size_ = x_.size
    ...
    for i in range(n_range_):
        ...
        for j in range(fcns_.size):
            fit = lsqfit.nonlinear_fit(data=(x_new_, y_new_), p0=p0_[j], fcn=fcns_[j], debug=False, svdcut=-1e-6)
